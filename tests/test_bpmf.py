...
from recommender.models.bpmf import BPMF
...
class TestBPMF(unittest.TestCase):
    ...
    def test_bpmf_with_random_data(self):
        ...
        bpmf1.fit(ratings, epochs=1)
        rmse_1 = RMSE().compute(bpmf1.predict(ratings[:, :2]), ratings[:, 2])
        ...
        bpmf2.fit(ratings, epochs=3)
        rmse_2 = RMSE().compute(bpmf2.predict(ratings[:, :2]), ratings[:, 2])
...
class TestBPMFwithMovieLens100K(unittest.TestCase):
    ...
    def test_bpmf_with_ml_100k_rating(self):
        ...
        bpmf.fit(ratings, epochs=30)
        rmse = RMSE().compute(bpmf.predict(ratings[:, :2]), ratings[:, 2])
