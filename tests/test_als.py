...
from rater.models.als import ALS
from rater.datasets.movielens import make_ratings
...
class TestALS(unittest.TestCase):
    ...
    def test_als_with_random_data(self):
        ...
        als1.fit(ratings, epochs=1)
        rmse_1 = RMSE().compute(als1.predict(ratings[:, :2]), ratings[:, 2])
        ...
        als2.fit(ratings, epochs=3)
        rmse_2 = RMSE().compute(als2.predict(ratings[:, :2]), ratings[:, 2])
...
class TestALSwithMovieLens100K(unittest.TestCase):
    def setUp(self):
        ...
        with gzip.open(file_path, 'rb') as f:
            ratings = pickle.load(f, encoding='latin1')
    ...
    def test_als_with_ml_100k_rating(self):
        ...
        als.fit(ratings, epochs=5)
        rmse = RMSE().compute(als.predict(ratings[:, :2]), ratings[:, 2])
