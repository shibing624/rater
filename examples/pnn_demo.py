...
from rater.models.ctr.pnn import PNN
...
def train(x_idx, x_value, label, features, out_type='binary'):
    ...
    model = PNN(emb_dim=5, num_feats=features.get_num_feats(), num_fields=features.get_num_fields(),
                out_type=out_type).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)

    model_path = os.path.join(pwd_path, 'pnn_model.pt')
