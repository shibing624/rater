...
from ...utils.partition import partition_num
...
class BiasedWalker:
    ...
    def _exec_random_walk(self, graphs, layers_accept, layers_alias, v, walk_length, gamma, stay_prob=0.3):
        ...
        _path = [self.idx2node[v]]

        while len(_path) < walk_length:
            r = random.random()
            if r < stay_prob:  # same layer
                v = choose_neighbor(v, graphs, layers_alias,
                                    layers_accept, layer)
                _path.append(self.idx2node[v])
...
def choose_neighbor(v, graphs, layers_alias, layers_accept, layer):
