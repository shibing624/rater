...
class NFM(torch.nn.Module):
    ...
    def __init__(self, field_size, feature_sizes, embedding_size=4, is_shallow_dropout=True, dropout_shallow=[0.5],
                 ...
                 use_fm=True, use_ffm=False, is_interaction=True, eval_metric=roc_auc_score,
    ...
    def forward(self, Xi, Xv):
        ...
        if self.use_ffm:
            ...
            ffm_second_order = sum(ffm_arr)

        """
            deep part
        """
        if self.use_fm and self.is_interaction:
            deep_emb = fm_second_order
        elif self.use_ffm and self.is_interaction:
            deep_emb = ffm_second_order
        elif self.use_fm:
            deep_emb = torch.cat([torch.sum(fm_wij, 1).view([-1, 1]) for fm_wij in fm_arr], 1)
        else:
            deep_emb = torch.cat([torch.sum(ffm_wij, 1).view([-1, 1]) for ffm_wij in ffm_arr], 1)
    ...
    def inner_predict(self, Xi, Xv):
        ...
        :return: output, numpy
        """
        return self.inner_predict_proba(Xi, Xv) > 0.5
...
if __name__ == '__main__':
    ...
    m = NFM(39, result_dict['feature_sizes'], use_cuda=True,
